...
- `requests` for Microsoft Graph HTTP requests.
...
Always inspect autogenerated migrations before applying them, especially for renamed columns, data transformations, PostgreSQL enum changes, and destructive operations. In production, run migrations as a separate release step before starting the crawler.
